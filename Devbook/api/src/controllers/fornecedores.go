...
// CriarFornecedor insere um fornecedor no banco de dados
func CriarFornecedor(w http.ResponseWriter, r *http.Request) {
	...
	if erro = json.Unmarshal(corpoRequest, &fornecedor); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}

	if erro = fornecedor.Preparar(); erro != nil {
		respostas.Erro(w, http.StatusBadRequest, erro)
		return
	}
	...
	repositorio := repositorios.NovoRepositorioDeFornecedores(db)
	fornecedor.ID, erro = repositorio.Criar(fornecedor)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}

	respostas.JSON(w, http.StatusCreated, fornecedor)
}

// BuscarUsuarios busca todos os usuários salvos no banco
func BuscarFornecedores(w http.ResponseWriter, r *http.Request) {
...
func BuscarFornecedor(w http.ResponseWriter, r *http.Request) {
